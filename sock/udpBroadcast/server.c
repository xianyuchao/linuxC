...
int main(int argc,char *argv[])
{
   ...
   setsockopt(socket_fd,SOL_SOCKET,SO_BROADCAST,&so_broadcast,sizeof(so_broadcast));
    ...
    sendto(socket_fd,buf,strlen(buf),0,(struct sockaddr *)&my_addr,sizeof(my_addr));//发广播
    size=sizeof(user_addr);
    recvfrom(socket_fd,buf,MAXDATASIZE,0,(struct sockaddr *)&user_addr,&size);//接收
    ...
    while(1) {
        ...
        if((sendto(socket_fd,buf,strlen(buf),0,//发广播
